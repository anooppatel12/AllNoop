...
export const useMultiWebRTC = (roomId: string) => {
  ...
  useEffect(() => {
    ...
    return () => {
      off(peersRef);
      if (myPeerIdRef.current) {
        const myAnswerRef = ref(database, `video-rooms/${roomId}/offers/${myPeerIdRef.current}`);
        off(myAnswerRef);
        const myIceCandidatesRef = ref(database, `video-rooms/${roomId}/iceCandidates/${myPeerIdRef.current}`);
        off(myIceCandidatesRef);
      }
      leaveRoom();
    };
