...
export function VideoLobby() {
  ...
  const handleCopyToClipboard = () => {
    if (!newRoomId) return;
    const url = `${window.location.origin}/video-room/${newRoomId}`;
  ...
  return (
    <Card className="mt-8">
      <CardHeader>
        ...
        <CardDescription>
          Create a new private video room or enter an existing room ID to join.
        </CardDescription>
      </CardHeader>
        ...
        {newRoomId && (
          <div className="rounded-lg border bg-muted p-4 space-y-3">
            ...
            <div className="flex gap-2">
              <Input value={`${window.location.origin}/video-room/${newRoomId}`} readOnly />
              <Button size="icon" onClick={handleCopyToClipboard}>
                {hasCopied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
              </Button>
            </div>
            <Button onClick={() => router.push(`/video-room/${newRoomId}`)} className="w-full">
