
'use client';

import { useState } from 'react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from '@/components/ui/card';
import { Label } from '@/components/ui/label';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { Terminal } from 'lucide-react';

const atomicWeights: { [key: string]: number } = {
  H: 1.008, He: 4.0026, Li: 6.94, Be: 9.0122, B: 10.81, C: 12.011, N: 14.007, O: 15.999, F: 18.998, Ne: 20.180,
  Na: 22.990, Mg: 24.305, Al: 26.982, Si: 28.085, P: 30.974, S: 32.06, Cl: 35.45, Ar: 39.948, K: 39.098, Ca: 40.078,
  Sc: 44.956, Ti: 47.867, V: 50.942, Cr: 51.996, Mn: 54.938, Fe: 55.845, Co: 58.933, Ni: 58.693, Cu: 63.546, Zn: 65.38,
  Ga: 69.723, Ge: 72.630, As: 74.922, Se: 78.971, Br: 79.904, Kr: 83.798, Rb: 85.468, Sr: 87.62, Y: 88.906, Zr: 91.224,
  Nb: 92.906, Mo: 95.96, Tc: 98, Ru: 101.07, Rh: 102.91, Pd: 106.42, Ag: 107.87, Cd: 112.41, In: 114.82, Sn: 118.71,
  Sb: 121.76, Te: 127.60, I: 126.90, Xe: 131.29, Cs: 132.91, Ba: 137.33, La: 138.91, Ce: 140.12, Pr: 140.91, Nd: 144.24,
  Pm: 145, Sm: 150.36, Eu: 151.96, Gd: 157.25, Tb: 158.93, Dy: 162.50, Ho: 164.93, Er: 167.26, Tm: 168.93, Yb: 173.05,
  Lu: 174.97, Hf: 178.49, Ta: 180.95, W: 183.84, Re: 186.21, Os: 190.23, Ir: 192.22, Pt: 195.08, Au: 196.97, Hg: 200.59,
  Tl: 204.38, Pb: 207.2, Bi: 208.98, Po: 209, At: 210, Rn: 222, Fr: 223, Ra: 226, Ac: 227, Th: 232.04, Pa: 231.04, U: 238.03,
  Np: 237, Pu: 244, Am: 243, Cm: 247, Bk: 247, Cf: 251, Es: 252, Fm: 257, Md: 258, No: 259, Lr: 262, Rf: 267, Db: 270,
  Sg: 271, Bh: 270, Hs: 277, Mt: 276, Ds: 281, Rg: 280, Cn: 285, Nh: 284, Fl: 289, Mc: 288, Lv: 293, Ts: 294, Og: 294,
};

export function MolecularWeightCalculator() {
  const [formula, setFormula] = useState('');
  const [molecularWeight, setMolecularWeight] = useState<number | null>(null);
  const [error, setError] = useState<string | null>(null);

  const parseFormula = (formulaStr: string): { [key: string]: number } | null => {
    const tokens = formulaStr.replace(/ /g, '').match(/([A-Z][a-z]*)(\d*)|(\()|(\))(\d*)/g);
    if (!tokens) return null;

    const stack: { [key: string]: number }[] = [{}];
    
    for (const token of tokens) {
        if (token === '(') {
            stack.push({});
        } else if (token.startsWith(')')) {
            const top = stack.pop();
            if (!top || !stack.length) return null; // Mismatched parentheses
            const multiplier = parseInt(token.substring(1) || '1', 10);
            for (const el in top) {
                stack[stack.length - 1][el] = (stack[stack.length - 1][el] || 0) + top[el] * multiplier;
            }
        } else {
            const match = token.match(/([A-Z][a-z]*)(\d*)/);
            if (!match) return null; // Invalid token
            const [, element, countStr] = match;
            const count = parseInt(countStr || '1', 10);
            if (!atomicWeights[element]) {
                throw new Error(`Unknown element: ${element}`);
            }
            stack[stack.length - 1][element] = (stack[stack.length - 1][element] || 0) + count;
        }
    }

    if (stack.length !== 1) return null; // Mismatched parentheses
    return stack[0];
  };

  const calculateWeight = () => {
    setError(null);
    setMolecularWeight(null);
    if (!formula.trim()) return;

    try {
        const atomCounts = parseFormula(formula);
        if (!atomCounts) {
            setError('Invalid formula structure. Check for mismatched parentheses or incorrect formatting.');
            return;
        }

        let totalWeight = 0;
        for (const element in atomCounts) {
            totalWeight += atomicWeights[element] * atomCounts[element];
        }
        
        setMolecularWeight(totalWeight);

    } catch (e: any) {
        setError(e.message);
    }
  };
  
  const resetCalculator = () => {
    setFormula('');
    setMolecularWeight(null);
    setError(null);
  }

  return (
    <Card className="mt-8">
      <CardHeader>
        <CardTitle>Molecular Weight Calculator</CardTitle>
        <CardDescription>Enter a chemical formula (e.g., H2O, C6H12O6, Mg(NO3)2).</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="space-y-2">
          <Label htmlFor="chemical-formula">Chemical Formula</Label>
          <Input
            id="chemical-formula"
            placeholder="Enter formula"
            value={formula}
            onChange={(e) => setFormula(e.target.value)}
            className="font-mono"
          />
        </div>
        <div className="flex gap-4">
          <Button onClick={calculateWeight} className="w-full">Calculate</Button>
          <Button onClick={resetCalculator} className="w-full" variant="outline">Reset</Button>
        </div>
      </CardContent>
      {molecularWeight !== null && (
        <CardFooter>
          <div className="w-full rounded-lg bg-muted p-4 text-center">
            <p className="text-lg">Total Molecular Weight:</p>
            <p className="text-3xl font-bold">{molecularWeight.toFixed(4)} g/mol</p>
          </div>
        </CardFooter>
      )}
      {error && (
        <CardFooter>
            <Alert variant="destructive">
                <Terminal className="h-4 w-4" />
                <AlertTitle>Error</AlertTitle>
                <AlertDescription>{error}</AlertDescription>
            </Alert>
        </CardFooter>
      )}
    </Card>
  );
}
